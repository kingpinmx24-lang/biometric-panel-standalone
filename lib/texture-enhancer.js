...
async function enhanceFingerprint(imageBuffer) {
  try {
    ...
    // Paso 5: Mejora de contraste adaptativa
    enhanced = await sharp(enhanced)
      .linear(1.8, -0.1)
      .toBuffer();
    ...
    // Paso 7: Limpieza de fondo (asegurar blanco puro en áreas claras)
    enhanced = await sharp(enhanced)
      .threshold(240, { grayscale: true })
      .negate() // Invertir para aplicar threshold al fondo
      .threshold(15, { grayscale: true })
      .negate()
      .toBuffer();
...
async function enhanceWithTexture(imageBuffer) {
  try {
    ...
    // Paso 5: Contraste para textura
    enhanced = await sharp(enhanced)
      .linear(1.8, -0.3)
      .toBuffer();
    
    // Paso 6: Median ligero para suavizar ruido
    enhanced = await sharp(enhanced)
      .median(1)
      .toBuffer();
...
async function ultraProfessionalEnhance(imageBuffer) {
  try {
    ...
    // Paso 5: Contraste ultra
    enhanced = await sharp(enhanced)
      .linear(2.5, -0.6)
      .toBuffer();
    
    // Paso 6: Median mínimo
    enhanced = await sharp(enhanced)
      .median(1)
      .toBuffer();
