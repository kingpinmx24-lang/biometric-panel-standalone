...
async function enhanceFingerprint(imageBuffer) {
  try {
    ...
    // Paso 5: Mejora de contraste adaptativa (en lugar de threshold agresivo)
    enhanced = await sharp(enhanced)
      .linear(1.5, -0.2) // Aumentar contraste
      .toBuffer();
    ...
    // Paso 7: Normalización final para asegurar rango dinámico
    enhanced = await sharp(enhanced)
      .normalize()
      .toBuffer();
